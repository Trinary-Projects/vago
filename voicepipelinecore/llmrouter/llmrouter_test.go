...
func TestBuildRequestOpenRouterGemmaProviderPreferences(t *testing.T) {
	...
	if !ok || len(order) != 4 || order[0] != "modelrun/fp4" || order[1] != "together" || order[2] != "parasail/fp8" || order[3] != "deepinfra/fp8" {
		t.Fatalf("provider.order = %#v", provider["order"])
	}
	only, ok := provider["only"].([]any)
	if !ok || len(only) != 4 || only[0] != "modelrun/fp4" || only[1] != "together" || only[2] != "parasail/fp8" || only[3] != "deepinfra/fp8" {
