...
- Sales calls wire `voicepipelinecore.LLMResponseTimeoutProcessor` after `LLMProcessor` and before TTS to mirror Disha backend `bots/onboarding_call/llm_response_timeout_processor.py`: start/reset a 7s timer on `LLMResponseStartFrame`, cancel on `LLMResponseEndFrame` or `InterruptFrame`, and on timeout broadcast `InterruptFrame` plus push `LLMMessagesAppendFrame(nil, true)` upstream as the existing Go equivalent of Pipecat `LLMRunFrame`. Do not add a separate retry frame type for this.
