...
| `internal/worker/lifecycle.go` | Worker lifecycle helpers. Worker registration checks/writes `registered_pod:{pod_name}:{pod_uid}` and enqueues `bots.gke_pod_manager.register_worker_pod_db_ops`; call cleanup enqueues `bots.signal_handler.cleanup_state`; pod env/IP detection lives here. It accepts `disha.Deps` but is outside `disha/` because this is worker-host lifecycle, not bot business logic |
